...
class LedgerBuilder:
    ...
    def build_standard_trace(self, *, kickoff_time=None, lock_time=None, sportmonks=None, supabase=None, polymarket=None, bookmaker=None, lineup=None, halftime=None, probability=None, consensus=None, edge=None, risk=None, prediction=None, order=None, reflection=None) -> list[dict]:
        ...
        central_record = None
        llm_central = (probability or {}).get("llm_central") or {}
        provider_result = llm_central.get("provider_result") or {}
        parsed_central = (provider_result or {}).get("parsed") or {}
        if provider_result:
            usage = provider_result.get("usage") or {}
            central_record = self.add("Thinking", "LLM central forecast synthesis", {"model_invocation": model_invocation(provider=provider_result.get("provider", "anthropic"), model_name=provider_result.get("model", "unknown"), internal_reasoning=provider_result.get("internal_reasoning") or summarize_reasoning(label="LLM central synthesis", inputs={"mode": (probability or {}).get("decision_mode"), "deterministic_reference": (probability or {}).get("deterministic_reference")}, outputs={"probabilities": parsed_central.get("probabilities"), "recommendation": parsed_central.get("recommendation"), "risk_posture": parsed_central.get("risk_posture")}, rule="LLM is the primary forecaster in llm_central mode, but deterministic safety gates still control order placement."), tokens_in=usage.get("input_tokens"), tokens_out=usage.get("output_tokens")), "prompt": "Synthesize all features into final home/draw/away probabilities and risk posture.", "inputs": [{"input_record_id": pid, "input_payload": "see upstream feature records"} for pid in [sm, sb, pm, lu] + ([ht] if ht else []) + ([claim_record] if claim_record else [])], "output_payload": llm_central, "reasoning_trace": reasoning_trace(step_type="llm_forecast", objective="Use all gathered features to produce the primary forecast.", method="Structured Anthropic Sonnet JSON forecast synthesis.", decision_rule="LLM forecast may set the primary probabilities in llm_central mode, but cannot bypass deterministic edge or risk gates.", output_summary=f"recommendation={llm_central.get('recommendation')}; fallback={llm_central.get('used_fallback')}", evidence_refs=[sm, sb, pm, lu] + ([ht] if ht else []) + ([claim_record] if claim_record else []), risk_controls=llm_central.get("blocking_risk_flags", []))}, parents=[sm, sb, pm, lu] + ([ht] if ht else []) + ([claim_record] if claim_record else []))
        ...
        pred = self.add("Acting", "prediction", {"action_type": "prediction", "target_system": "arena", "action_summary": f"Submit latest {self.window} probability distribution for scoring.", "parameters": prediction_params, "dry_run": self.settings.dry_run, "execution_status": "dry_run" if self.settings.dry_run else "confirmed", "reasoning_trace": reasoning_trace(step_type="action", objective="Emit prediction record for PSL scoring.", inputs_used=["final_probs", "confidence", "risk_flags"], method="Acting record with full distribution plus top outcome/probability.", decision_rule="Always submit prediction even when order is skipped.", output_summary=f"outcome={outcome}; probability={probability_value}", evidence_refs=[ra], risk_controls=["probabilities normalized", "prediction separate from order"])}, parents=[ra])
        order_action = (order or {}).get("action_type", "skip")
        order_status = (order or {}).get("final_status") or (order or {}).get("status")
        if self.settings.dry_run:
            execution_status = "dry_run"
        elif order_action != "order":
