...
def bet_size(edge_tier: str, confidence: float, max_order_usd: float, consensus_modifier: float = 1.0, allow_soft: bool = False) -> float:
    ...
    if edge_tier == "soft": base = 0.75 if allow_soft and confidence >= .68 else 0.0
    elif edge_tier == "medium": base = 1.35 + max(0.0, confidence-.53)*5.5
    else: base = 2.9 + max(0.0, confidence-.53)*4.5
